...
| 7 | `GET` | `/Filter1Hour/{city}/{country}/{day}/{time}` | *Restituisce un oggetto di tipo WeatherStats contenente le statistiche di una città filtrate relativamente ad un'ora precisa di un giorno, entrambi scelti dall'utente.* |
...
La seguente rotta restituisce un oggetto di tipo City contenente le informazioni sulla velocità del vento e la data e l'ora a cui le previsioni si riferiscono. Nel caso in cui la città di Ancona e il suo Paese non siano scritti correttamente, riceverete un messaggio di errore.
